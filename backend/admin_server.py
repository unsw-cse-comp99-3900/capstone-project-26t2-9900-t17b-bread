...
try:
    ...
    with engine.begin() as conn:
        ...
        if "review_status" not in existing_columns:
            conn.execute(text(f"ALTER TABLE {TABLE_NAME} ADD COLUMN review_status VARCHAR(50) DEFAULT 'pending';"))
            print(f"[DB Patch] Successfully added 'review_status' column to {TABLE_NAME}.")
        else:
            print(f"'review_status' column already exists in {TABLE_NAME}.")
        
        if "admin_notes" not in existing_columns:
            conn.execute(text(f"ALTER TABLE {TABLE_NAME} ADD COLUMN admin_notes TEXT;"))
            print(f"[DB Patch] Successfully added 'admin_notes' column to {TABLE_NAME}.")
        else:
            print(f"'review_status' column already exists in {TABLE_NAME}.")

        count_result = conn.execute(text(f"SELECT COUNT(*) FROM {TABLE_NAME};"))
        data_count = count_result.scalar()
        
        if data_count == 0:
            conn.execute(text(f"""
                INSERT INTO {TABLE_NAME} (review_status, admin_notes, result_json, article_a_id, article_b_id) 
                VALUES ('pending', 'Full-stack integration test: This is mock review data automatically generated by the shadow backend.', '{{}}', 1, 2);
            """))
            print(f"[DB Patch] Table {TABLE_NAME} is empty. Successfully injected 1 test mock data row!")
        else:
            print(f"[DB Patch] Table {TABLE_NAME} currently has {data_count} rows. Skipping injection.")

except Exception as db_err:
    print(f"[DB Patch Warning] Error checking or patching admin columns: {db_err}")
